...
class GitHubService:
    ...
    def add_reaction_to_issue_comment(self, comment_id: int, reaction_type: str) -> None:
        try:
            comment = self.github_repo.get_issue_comment(comment_id)
            comment.create_reaction(reaction_type)
        except UnknownObjectException as e:
            logger.error(f"Failed to add reaction to issue comment: {e}")
            raise GitHubServiceException("Failed to add reaction to issue comment") from e

    def add_reaction_to_pull_request_review_comment(self, comment_id: int, reaction_type: str) -> None:
        try:
            comment = self.github_repo.get_pull_request_review_comment(comment_id)
            comment.create_reaction(reaction_type)
        except UnknownObjectException as e:
            logger.error(f"Failed to add reaction to pull request review comment: {e}")
            raise GitHubServiceException("Failed to add reaction to pull request review comment") from e
